...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		String API_KEY = "bc3e308cebda12198bf8d5d160dbc282";
		...
		Document doc = convertStringToXMLDocument(ApiResponse, request, response);
		...
		// Create a Node list that gets everything in and under the "clouds" tag
		NodeList mList = doc.getElementsByTagName("lastupdate");
		...
		for (int temp = 0; temp < mList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				// and print it
				System.out.println(wBean.getCityStr() + " is now a " + XMLlastupdate);
				// save it
	...
	private static Document convertStringToXMLDocument(String xmlString, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
